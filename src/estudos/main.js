...
// Sugestões para métodos
if (this.methodsAndFunctions[input]) {
const methodSuggestions = this.methodsAndFunctions[input].map(method => ({
caption: method,
value: `${input}.${method}()`,
score: 800,
meta: "Method",
...
// Processar módulos válidos e extrair métodos e funções
async processModules() {
console.log("passou aqui: processModules");
try {
const foundModules = this.filterValidModules(this.existingModules);

// Processar cada módulo válido
for (const module of foundModules) {
// Carregar o módulo dinamicamente
const moduleInstance = require(module);

// Extrair métodos e funções do módulo
const methodsAndFunctions = Object.keys(moduleInstance).filter(key => {
return typeof moduleInstance[key] === 'function' || typeof moduleInstance[key] === 'object';
});

// Armazenar os métodos e funções
this.methodsAndFunctions[module] = methodsAndFunctions;
}

// Armazenar foundModules para uso em extractMethodsAndFunctions
this.foundModules = foundModules;
console.log('Métodos e Funções:', this.methodsAndFunctions);
} catch (error) {
console.log('Erro ao processar módulos', error);
}
}
